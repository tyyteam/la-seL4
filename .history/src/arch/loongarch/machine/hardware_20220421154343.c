...
{
	eentry    = (unsigned long)exception_handlers;//QT 普通例外入口
	tlbrentry = (unsigned long)exception_handlers + 80*VECSIZE;//tlb例外入口

	csr_writeq(eentry, LOONGARCH_CSR_EENTRY);//入口写入寄存器
